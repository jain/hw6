...
'''[Selector,
 [Sequence, ChaseHero, KillHero],
 ]
] '''

pct = 0.4
#AREAEFFECTRANGE = max(AREAEFFECTRANGE, 30)
def treeSpec(agent):
	myid = str(agent.getTeam())
	spec = None
	### YOUR CODE GOES BELOW HERE ###
	global pct
	spec = [Selector, Retreat, KillHero, KillMinion, ChaseMinion, ChaseHero]# \
	...
	def execute(self, delta = 0):
		...
		if len(enemies) > 0:
			for e in enemies:
				...
				if d <= float(AREAEFFECTRANGE) + e.maxradius:
...
class KillMinion(BTNode):
	...
	def enter(self):
		...
		self.agent.navigator.setPath(None)
		enemies = self.agent.world.getEnemyNPCs(self.agent.getTeam())
		if len(enemies) > 0:
			best = None
			dist = 0
	...
	def execute(self, delta = 0):
		...
		if len(enemies) > 0:
			for e in enemies:
				...
				if isinstance(e, Hero):
					self.target = e
					if distance(self.agent.getLocation(), e.getLocation()) <= BIGBULLETRANGE:
						self.agent.turnToFace(e.getLocation())
						self.agent.shoot()
						return False
		if self.agent.getHitpoints() <= self.agent.getMaxHitpoints() * self.percentage:
		...
		if self.target == None or distance(self.agent.getLocation(), self.target.getLocation()) > BIGBULLETRANGE:
			...
			# executing
			self.agent.dodge(random.randint(0, 359))
			self.shootAtTarget()
			return None
		return ret
	...
	def shootAtTarget(self):
		if self.agent is not None and self.target is not None:
			deg = 90+ angle(self.agent.getLocation(), self.target.getLocation())
			self.agent.dodge(deg)
